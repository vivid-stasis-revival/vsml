    ...
    public void Execute()
    {
        ...
        if (Directory.Exists(_codeReplacePath))
        {
            ...
            foreach (var file in pendingFiles)
            {
                ...
                ConsoleOutput.PrintInfo($"处理代码文件: {codeName}", $"Processing code file: {codeName}");
                ...
                if (codeName.StartsWith(objectPrefix, StringComparison.Ordinal))
                {
                    ...
                    if (!manualLink)
                    {
                        batch.QueueReplace(codeName, code);
                        _replacedEntries.Add(codeName);
                        batchCount++;
                        if (batchCount >= batchSize)
                        {
                            var _result = batch.Import();
                            ...
                            batch = new CodeImportGroup(data) { AutoCreateAssets = true };
                            batchCount = 0;
                        }
                        continue;
                    }
                    ...
                    CodeImportGroup.LinkEvent(obj, manualCode, EventType.Collision, eventSubtype, batch.MainThreadAction);
                    // Perform code import using manual code entry
                    batch.QueueReplace(manualCode, code);
                    _replacedEntries.Add(codeName);
                    batchCount++;
                    if (batchCount >= batchSize)
                    {
                        var _manualResult = batch.Import();
                        if (!_manualResult.Successful)
                        {
                            ConsoleOutput.PrintError("代码导入失败。", "Code import unsuccessful.");
                            ConsoleOutput.PrintError(_manualResult.PrintAllErrors(false), _manualResult.PrintAllErrors(false));
                        }
                        batch = new CodeImportGroup(data) { AutoCreateAssets = true };
                        batchCount = 0;
                    }
                ...
                else
                {
                    ...
                    if (batchCount >= batchSize)
                    {
                        var _elseResult = batch.Import();
                        if (!_elseResult.Successful)
                        {
                            ConsoleOutput.PrintError("代码导入失败。", "Code import unsuccessful.");
                            ConsoleOutput.PrintError(_elseResult.PrintAllErrors(false), _elseResult.PrintAllErrors(false));
                        }
                        batch = new CodeImportGroup(data) { AutoCreateAssets = true };
                        batchCount = 0;
                    }
            ...
            if (batchCount > 0)
            {
                var finalResult = batch.Import();
                if (!finalResult.Successful)
                {
                    ConsoleOutput.PrintError("代码导入失败。", "Code import unsuccessful.");
                    ConsoleOutput.PrintError(finalResult.PrintAllErrors(false), finalResult.PrintAllErrors(false));
                }
            }
        }
